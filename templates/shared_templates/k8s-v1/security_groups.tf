resource "aws_security_group" "k8s_master_instance" {
  name        = "${var.region}_${var.aws_account_short_name}-master-instance"
...
resource "aws_security_group" "k8s_worker_instance" {
  name        = "${var.region}_${var.aws_account_short_name}-worker-instance"
...
resource "aws_security_group" "k8s_etcd_instance" {
  name        = "${var.region}_${var.aws_account_short_name}-kubernetes-etcd-instance"
  ...
  tags {
    Name               = "${var.region}_${var.aws_account_short_name}-etcd-instance"
    Pipeline_phase     = "${var.env}"
    Jive_service       = "${var.jive_service}"

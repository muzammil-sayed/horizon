...
resource "aws_autoscaling_group" "k8s_master" {
  name                 = "${var.env}-premerge-k8s-master-asg${count.index+1}"
  ...
  tag {
    key                 = "Name"
    value               = "${var.env}-premerge-k8s-master-instance"
...
resource "aws_ebs_volume" "k8s_master" {
  ...
  tags {
    Name              = "${var.env}-premerge-k8s_master-instance"
...
resource "aws_elb" "k8s_master" {
  name            = "${var.env}-premerge-k8s-master-elb"
  security_groups = ["${var.aws_security_group_env_instance}", "${aws_security_group.k8s_master_instance.id}"]
  ...
  tags {
    Name              = "${var.env}-premerge-k8s_master-elb"
    Hostname          = "${null_resource.k8s_cluster.triggers.name}-master${count.index+1}"
    Pipeline_phase    = "${var.env}"
    Jive_service      = "${null_resource.k8s_cluster.triggers.name}"
    Region            = "${var.region}"
...
resource "aws_route53_record" "premerge-pipeline-us-master-elb" {
  zone_id = "${var.route53_zone_id}"
  name    = "premerge-pipeline-us-master"
  type    = "CNAME"
  ttl     = "60"

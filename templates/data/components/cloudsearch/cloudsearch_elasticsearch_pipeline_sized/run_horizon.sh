...
if [ -z "$environment" ]
then
  echo "[ERROR] Please specify an environment (integ|test|preprod|prod)"
  exit 1
fi
...
echo "${path_to_horizon} -A ${action} -a ${account_name} -e $environment -n infra-pipeline -p ${rest_of_path} -r administrator --regions us-west-2 -v"
# echo -n "Execute [y/N]? "
# read resp
...
#   exit
# else
${path_to_horizon} -A ${action} -a ${account_name} -e $environment -n infra-pipeline -p ${rest_of_path} -r administrator --regions us-west-2 -v
# fi

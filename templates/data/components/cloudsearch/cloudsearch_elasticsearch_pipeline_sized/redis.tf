resource "aws_security_group_rule" "redis_port_6379" {
  ...
  security_group_id = "${aws_security_group.integ_redis_ports.id}"
}
...
resource "aws_instance" "search_redis1" {
  ...
  vpc_security_group_ids      = ["${var.aws_security_group_env_instance}", "${aws_security_group.integ_redis_ports.id}"]

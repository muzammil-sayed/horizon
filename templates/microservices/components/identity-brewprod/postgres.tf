resource "aws_db_parameter_group" "id-pg-brewprod" {
    name = "${var.postgres_parameter_group}"
    family = "postgres9.5"
    }


resource "aws_db_instance" "pg-instance-id-brewprod" {
...
resource "aws_db_subnet_group" "postgres" {
  name        = "soa-pg-${var.postgres_subnet_name}-brew-${var.mako_env}-group"
